 ...
 * vfs_new_file
 *
 * Sends a request for a new file to be created in driver <root> at path
 * <path>. Returns a file descriptor of the new file on success, NULL on
 * failure.
 */
...
FILE *vfs_new_file(FILE *root, const char *path) {
	...
	query.opcode = VFS_ACT | VFS_NEW | VFS_FILE;
