...
static int elf_null_segment(elf_ph_t *segment);
static int elf_load_segment(elf_ph_t *segment);
static int elf_dynm_segment(elf_ph_t *segment);
static int elf_intr_segment(elf_ph_t *segment);
static int elf_note_segment(elf_ph_t *segment);
static int elf_shar_segment(elf_ph_t *segment);
static int elf_phdr_segment(elf_ph_t *segment);

typedef int (*elf_segment_handler_t) (elf_ph_t *segment);
...
int elf_segment_load(elf_t *header, elf_ph_t *segment) {
	return elf_segment_handler[segment->p_type](segment);
}
...
static int elf_load_segment(elf_t *header, elf_ph_t *segment) {
	u8int *mem_base = segment->p_vaddr;
	...
	// Copy data
	memcpy(mem_base, file_base[file_off], segment->p_memsz);
